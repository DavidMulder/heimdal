...
static char* qas_krb5_string_strstripL(const char* start)
{
...
int qas_krb5_readline( FILE* input, 
                        char** buffer, 
                        size_t* size_ptr )
{
    int   rval = -1;
    ...
    while( 1 )
    {
        ...
        if( fgets( runner, available_size, input ) != NULL )
        {
            ...
            if( trim_white_left )
            {
                ...
                {
                    char* trimmed_buff;
                    if( (trimmed_buff = qas_krb5_string_strstripL( runner )) == NULL )
                    {
                        errno = EINVAL;
                        goto CLEANUP;
                    }
                         ...
                         int can_resize_line )
{
    ...
    keylen = strlen( key );
    while( (bytesread = qas_krb5_readline( input,
                                           myline_ptr,
                                           mylinesize_ptr )) >= 0 )
    {
        slider = *myline_ptr;
